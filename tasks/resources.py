...
class YFiChart(ResourceBase):
    ...
    @property
    def url(self):
        return f"https://query2.finance.yahoo.com/v8/finance/chart/{self.ticker}"

    def __init__(self, ticker:str, period:int, headers:dict=HEADERS, timeout:int=10000):
        """
        Args:
            period `range`(str):
    ...
    def get(self) -> List[dict]:
        ...
        if response.status_code != 200:
            print(f"request failed for ticker {self.ticker} with status code {response.status_code}")
            return []
        rj = response.json()
        ...
        days_tup = zip([self.ticker] * len(timestamps), timestamps, closes)
        ticker_days = [
